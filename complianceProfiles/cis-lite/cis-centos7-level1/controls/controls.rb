...
control "xccdf_org.cisecurity.benchmarks_rule_1.7_Use_the_Latest_OS_Release" do
  ...
  desc  "Periodically, CentOS releases updates to the CentOS operating system to support new hardware platforms, deliver new functionality as well as the bundle together a set of patches that can be tested as a unit."
  impact 0.0
end
...
control "xccdf_org.cisecurity.benchmarks_rule_3.2_Remove_the_X_Window_System" do
  ...
  describe file("/etc/systemd/system/default.target").link_target do
...
control "xccdf_org.cisecurity.benchmarks_rule_7.2_Disable_System_Accounts" do
  ...
  desc  "There are a number of accounts provided with the CentOS that are used to manage applications and are not intended to provide an interactive shell."
  impact 1.0
...
control "xccdf_org.cisecurity.benchmarks_rule_9.2.2_Verify_No_Legacy__Entries_Exist_in_etcpasswd_File" do
  ...
  desc  "The character + in various files used to be markers for systems to insert data from NIS maps at a certain point in a system configuration file. These entries are no longer required on CentOS 7 systems, but may exist in files that have been imported from other platforms."
  impact 1.0
  describe file("/etc/passwd") do
...
control "xccdf_org.cisecurity.benchmarks_rule_9.2.10_Check_for_Presence_of_User_.rhosts_Files" do
  ...
  desc  "While no .rhosts files are shipped with CentOS 7, users can easily create them."
  impact 1.0
  describe "SCAP oval resource file_test could not be loaded: Cannot handle referenced value group in file_test; only single values are support at the moment" do
    skip "SCAP oval resource file_test could not be loaded: Cannot handle referenced value group in file_test; only single values are support at the moment"
  end
...
control "xccdf_org.cisecurity.benchmarks_rule_9.2.16_Check_That_Reserved_UIDs_Are_Assigned_to_System_Accounts" do
  ...
  impact 1.0
  describe passwd.users(/^(?!root|bin|daemon|adm|lp|sync|shutdown|halt|mail|news|uucp|operator|games|gopher|ftp|nobody|nscd|vcsa|rpc|mailnull|smmsp|pcap|ntp|dbus|avahi|sshd|rpcuser|nfsnobody|haldaemon|avahi-autoipd|distcache|apache|oprofile|webalizer|dovecot|squid|named|xfs|gdm|sabayon|usbmuxd|rtkit|abrt|saslauth|pulse|postfix|tcpdump).*$/) do
    its(:lines) { should_not be_empty  }
  end
end
