...
control "xccdf_org.cisecurity.benchmarks_rule_1.2.1_Configure_Connection_to_the_RHN_RPM_Repositories" do
  title "Configure Connection to the RHN RPM Repositories"
  desc  "Systems need to be registered with the Red Hat Network (RHN) to receive patch updates. This is usually configured during initial installation."
  impact 0.0
end

control "xccdf_org.cisecurity.benchmarks_rule_1.2.2_Verify_Red_Hat_GPG_Key_is_Installed" do
  title "Verify Red Hat GPG Key is Installed"
...
control "xccdf_org.cisecurity.benchmarks_rule_1.2.3_Verify_that_gpgcheck_is_Globally_Activated" do
...
control "xccdf_org.cisecurity.benchmarks_rule_1.2.5_Obtain_Software_Package_Updates_with_yum" do
...
control "xccdf_org.cisecurity.benchmarks_rule_1.2.6_Verify_Package_Integrity_Using_RPM" do
...
control "xccdf_org.cisecurity.benchmarks_rule_1.5.1_Set_UserGroup_Owner_on_bootgrub2grub.cfg" do
  ...
  desc  "Set the owner and group of /boot/grub2/grub.cfgto the root user."
  impact 1.0
...
control "xccdf_org.cisecurity.benchmarks_rule_2.1.15_Disable_daytime-stream" do
  ...
  desc  "daytime-stream is a network service that responds with the server's current date and time. This service is intended for debugging and testing purposes. It is recommended that this service be disabled."
  impact 1.0
  describe.one do
...
control "xccdf_org.cisecurity.benchmarks_rule_3.2_Remove_the_X_Window_System" do
  ...
  describe "SCAP oval resource symlink_test is not yet supported." do
    skip "SCAP oval resource symlink_test is not yet supported."
  end
end
...
control "xccdf_org.cisecurity.benchmarks_rule_6.3.1_Upgrade_Password_Hashing_Algorithm_to_SHA-512" do
  ...
  desc  "The SHA-512 encryption has been available since Red Hat release 5.2,. The commands below change password encryption from md5 to sha512 (a much stronger hashing algorithm). All existing accounts will need to perform a password change to upgrade the stored hashes to the new algorithm."
  impact 1.0
...
control "xccdf_org.cisecurity.benchmarks_rule_6.3.2_Set_Password_Creation_Requirement_Parameters_Using_pam_pwquality" do
  ...
  desc  "The pam_pwquality module checks of the strength of passwords. It performs checks such as making sure a password is not a dictionary word, it is a certain length, contains a mix of characters (e.g. alphabet, numeric, other) and more. The following are definitions of the pam_pwquality.so options.\n               \n                  \n                     try_first_pass - retrieve the password from a previous stacked PAM module. If not available, then prompt the user for a password.\n                  \n                     \n                        retry=3 \n                     - Allow 3 tries before sending back a failure.\n               \n               The following options are set in the /etc/security/pwquality.conf file:\n               \n                  \n                     minlen=14 - password must be 14 characters or more\n                  \n                     dcredit=-1 - provide at least 1 digit\n                  \n                     ucredit=-1 - provide at least one uppercase character\n                  \n                     ocredit=-1 - provide at least one special character\n                  \n                     lcredit=-1 - provide at least one lowercase character\n               \n               The setting shown above is one possible policy. Alter these values to conform to your own organization's password policies."
  impact 1.0
  describe file("/etc/pam.d/system-auth") do
...
control "xccdf_org.cisecurity.benchmarks_rule_7.2_Disable_System_Accounts" do
  ...
  describe passwd.users(/^(?!root|sync|shutdown|halt).*$/).uids({:<==>1000}).shells({:!==>"/sbin/nologin"}) do
...
control "xccdf_org.cisecurity.benchmarks_rule_9.2.2_Verify_No_Legacy__Entries_Exist_in_etcpasswd_File" do
  ...
  desc  "The character + in various files used to be markers for systems to insert data from NIS maps at a certain point in a system configuration file. These entries are no longer required on RHEL7 systems, but may exist in files that have been imported from other platforms."
  impact 1.0
  describe file("/etc/passwd") do
...
control "xccdf_org.cisecurity.benchmarks_rule_9.2.10_Check_for_Presence_of_User_.rhosts_Files" do
  ...
  desc  "While no .rhosts files are shipped with RHEL7, users can easily create them."
  impact 1.0
  describe "SCAP oval resource file_test could not be loaded: Cannot handle referenced value group in file_test; only single values are support at the moment" do
    skip "SCAP oval resource file_test could not be loaded: Cannot handle referenced value group in file_test; only single values are support at the moment"
  end
...
control "xccdf_org.cisecurity.benchmarks_rule_9.2.16_Check_That_Reserved_UIDs_Are_Assigned_to_System_Accounts" do
  ...
  desc  "Traditionally, UNIX systems establish \"reserved\" UIDs (0-999 range) that are intended for system accounts."
  impact 1.0
  describe "SCAP oval resource password_test could not be loaded: Don't know how to create inspec_filter for \"greater than or equal\"." do
    skip "SCAP oval resource password_test could not be loaded: Don't know how to create inspec_filter for \"greater than or equal\"."
  end
end
